<?
$url = "http://gateway.fortunelounge.com/feeds/rvwinnersnonprogressive.xml";
$ch = curl_init(); 
...
foreach($xml->children() as $data)
 ...
 $winner['date'] = trim($data->DateWon);
 $winner['player'] = trim($data->PlayerName);
 $winner['country'] = trim($data->PlayerCountry);
 $winner['currency'] = "USD";
 $winner['amount'] = trim($data->AmountWon);
 $winner['game'] = trim($data->GamePlayed);
 $winner['casino'] = trim($data->CasinoName);
